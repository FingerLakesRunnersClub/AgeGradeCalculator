...
public static class TrackGenerator
{
	...
	private static IEnumerable<DataPoint<TrackEvent, TimeSpan>> GetOpenAgeGrades(Dictionary<TrackKey, TimeSpan> worldRecords)
		=> worldRecords.SelectMany(r => Enumerable.Range(20, 11).Select(age => new DataPoint<TrackEvent, TimeSpan> { Category = r.Key.Category, Age = (byte)age, Event = r.Key.Event, Record = r.Value }));

	private static async Task<IEnumerable<DataPoint<TrackEvent, TimeSpan>>> GetJuniorAgeGrades()
	{
	...
	private static List<DataPoint<TrackEvent, TimeSpan>> ParseJuniorAgeGrades(Category category, string data)
	{
		var dataPoints = new List<DataPoint<TrackEvent, TimeSpan>>();
		...
		foreach (var e in events.TakeWhile(e => e.Length > 1))
		{
			...
			foreach (var line in lines.Skip(2).TakeWhile(l => l.Length > 11))
			{
				...
				var dataPoint = new DataPoint<TrackEvent, TimeSpan> { Category = category, Age = age, Event = discipline, Record = time };
	...
	private static IEnumerable<DataPoint<TrackEvent, TimeSpan>> CalculateAgeGrades(Dictionary<TrackKey, double> ageFactors, Dictionary<TrackKey, TimeSpan> worldRecords)
		=> ageFactors
			.Where(f => worldRecords.ContainsKey((f.Key.Category, 0, f.Key.Event)))
			.Select(f => new DataPoint<TrackEvent, TimeSpan> { Age = f.Key.Age, Category = f.Key.Category, Event = f.Key.Event, Record = TimeSpan.FromSeconds(worldRecords[(f.Key.Category, 0, f.Key.Event)].TotalSeconds / f.Value) });
